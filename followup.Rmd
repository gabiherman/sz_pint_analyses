...
```{r}
ds_pheno_pass <- read.csv("/projects/edickie/code/SZ_PINT/data/processed/pheno/20200214_pheno_qapass.csv", stringsAsFactors = FALSE) %>% mutate_all(trimws) %>% mutate_all(na_if,"NA") %>% mutate_all(na_if, "") %>% mutate_all(na_if, " ") 
...
```{r}
compiled <- merge(qa_list,panss, by=c("sub_id","dataset"),all.x=TRUE) %>% merge(., select(ZHH,dataset, sub_id, SessNo), by = c("sub_id", "dataset"), all.x=TRUE) %>% merge(.,sans, by=c("sub_id","dataset"),all.x=TRUE)  %>% merge(.,wtar, by=c("sub_id","dataset"),all.x=TRUE)  %>% 
...
```{r}
write.csv(all_compiled,"/projects/gherman/SZ_PINT/compiled_clinical/compiled_2020_feb24.csv")
```
