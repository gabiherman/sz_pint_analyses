...
```{r}
compiled <- merge(qa_list,panss, by=c("sub_id","dataset"),all.x=TRUE,all.y=TRUE) %>% merge(.,sans, by=c("sub_id","dataset"),all.x=TRUE,all.y=TRUE)  %>% merge(.,wtar, by=c("sub_id","dataset"),all.x=TRUE,all.y=TRUE)  %>% merge(., dx)
```
...
by_group %>% summarize(total= sum(!is.na(sub_id)), n_wtar = sum(!is.na(wtar_std)), n_sans= sum(!is.na(sans_G_total)), n_panss=sum(!is.na(panss_total_calc)) )
```
