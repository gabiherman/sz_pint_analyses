...
```
note to self: gotta deal with the shared zhh openneuro ids...
...
```{r}
by_group<- compiled %>% group_by(dataset)
  
  # group_by(dx, dataset)

by_group %>% summarize(total_participants= sum(!is.na(sub_id)), n_wtar = sum(!is.na(wtar_std)), n_sans= sum(!is.na(sans_G_total)), n_panss=sum(!is.na(panss_total_calc))) %>% bind_rows(summarise_all(., funs(if(is.numeric(.)) sum(.) else "Total")))
```
...
edu <- read.csv("/scratch/gherman/SCZ_PINT/data/saba/Demographics_Github_29Aug2018.csv", stringsAsFactors = FALSE) %>% 
  mutate(., sub_id =  case_when(
      ...
      select(sub_id, Edu, Site)


all_compiled <- compiled %>% left_join(., edu, by=c("sub_id","Site"))
```
