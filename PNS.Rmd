...
PNS_pheno_pass <- read.csv("/projects/edickie/code/SZ_PINT/phenotypic/20200202_pheno_qapass.csv", stringsAsFactors = FALSE) %>% filter(., dataset=="PNSC")%>% mutate_all(trimws) %>% mutate_all(na_if,"NA") %>% mutate_all(na_if, "") %>% mutate_all(na_if, " ") 
...
write.csv(final_list,'/projects/gherman/SZ_PINT/compiled_clinical/cleaner_data_sets/PNS_arranged2020.csv')
