...
```{r}
COBRE_pheno_pass <- read.csv("/projects/edickie/code/SZ_PINT/data/processed/pheno/20200202_pheno_qapass.csv", stringsAsFactors = FALSE) %>% filter(., dataset=="COBRE")%>% mutate_all(trimws) %>% mutate_all(na_if,"NA") %>% mutate_all(na_if, "") %>% mutate_all(na_if, " ") 
...
write.csv(final_final_list,'/projects/gherman/SZ_PINT/compiled_clinical/cleaner_data_sets/COBRE_arranged_2020.csv')
