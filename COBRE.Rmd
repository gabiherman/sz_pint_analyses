...
data <- data %>% distinct(., source, study, site, subjectid, visit, assessment, assessment_description, .keep_all=TRUE) %>% select(subjectid, question_id,question_value)
```

```{r}

data2 <- data %>% spread(key=question_id, value=question_value)
```


```{r}
data2 <- data2 %>% mutate(., imaging_pass= ifelse((subjectid %in% qa_list$subjectid), 1, 0))
